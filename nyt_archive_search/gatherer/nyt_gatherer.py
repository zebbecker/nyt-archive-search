...
def search_nyt(keyword: str, start_date: dt.date, end_date: dt.date, api_key: str, download_limit: int):
    """
...
def download_one_page(keyword: str, start_date: dt.date, end_date: dt.date, api_key: str, results_page: int, data: list, next_id: int):
    ...
    if page_meta.status_code != requests.codes.ok:
        raise RuntimeError("Request to NYT API returned status "+ str(page_meta.status_code) + ". Make sure API key is valid.")
